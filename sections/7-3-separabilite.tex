...
L'hypothèse sous-jacente est la suivante : plus les classes sont nettement délimitées sur le plan géométrique, plus l'identification non-supervisée de ces classes (et donc l'extraction de taxonomie) sera facile. Pour mesurer si deux classes sont bien délimitées, on mesure leur \textit{séparabilité linéaire}, ou plus précisément la séparabilité linéaire des plongements vectoriels de leurs instances. % de leurs plongements vectoriels.
%
...
%
La condition de séparabilité linéaire présente l'avantage d'être conceptuellement simple et facile à calculer, tout en imposant une condition forte sur les plongements.
trois propriétés intéressantes : elle est conceptuellement simple, facile à calculer, et impose une condition forte sur les plongements. 

la délimitation entre les classes, on choisit 


Deux groupes de points sont linéairement séparables s'il est possible de faire passer un hyperplan entre les deux; en deux dimensions, cela signifie que l'on peut trouver une droite telle que tous les points du premier groupe soient situés d'un côté de la droite, et tous les points du second groupe soient de l'autre côté.
Notre idée est de mesurer si cette condition est respectée entre les plongements vectoriels de deux classes différentes. %Le choix d'une séparabilité \textit{linéaire} est une exigence assez forte

Ici, notre hypothèse est que, plus les classes sont géométriquement délimitées, plus l'identification non-supervisée de ces classes (et donc l'extraction de taxonomie) sera facile. 
En choisissant une séparabilité \textit{linéaire}, on impose une condition assez forte
C'est pourquoi on choisit de mesurer une séparabilité \textit{linéaire}, qui constitue une condition assez forte, en plus d'être calculable efficacement.
...
Pour cette distance, les classes les plus proches sont \dbo{NascarDriver} et \dbo{RacingDriver}, 

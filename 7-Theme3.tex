...
\Parameter{nombre d'entités à prélever à chaque étape $N_e$, à compléter}
\SetKwFunction{Preleve}{Preleve}
\SetKwFunction{Regroupe}{Regroupe}
\SetKwFunction{TrouveAxiomes}{Trouve\_Axiomes}
...
\While{$A$ est non-vide}{
    $a \leftarrow $ premier axiome de la file $A$ \;
    ...
    $\Tpred \leftarrow $ \Fusionne{$\Tpred, T_a, a$} \;
    % $t \leftarrow t + 1$ \;
}
\Return{$X$}
...
\Fn{\Fusionne{$T_1, T_2, \alpha$}}{
    $(V_1, E_1) \leftarrow T_1$ \;
    $(V_2, E_2) \leftarrow T_2$ \;
    $V \leftarrow V_1 \cup V_2$ \;
    $E \leftarrow E_1 \cup E_2$ \;
    \If{$\alpha$ est de la forme \texttt{<?>}}{
        \For{$u, v \in V$ tels que $u \rightarrow \alpha \rightarrow v$}{
            \tcp{on relie directement u à v}
            $E \leftarrow \left(E \setminus \{ (u , \alpha), (\alpha, v) \}\right) \cup \{ (u, v) \} $ \;
        }
        $V \leftarrow V \setminus \{ \alpha \}$ \;
    }
    $T \leftarrow (V, E)$ \;
    \Return{$T$}
}
...
\Fn{\Fusionne{$X_a, \delta, D$}}{
    ...
    \While{\NT n'est pas vide}{
        ...
        \eIf{$\alpha(C)$ n'est pas \indef}{
            ...
            \If{la profondeur de $C$ est inférieure à $D$}{
...
\KwInput{un ensemble d'exemples positifs $E^+$ et négatifs $E^-$ (par exemple, le cluster cible et son voisin), et une liste d'atomes}

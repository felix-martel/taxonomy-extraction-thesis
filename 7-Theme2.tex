...
Alternativement, on pourrait choisir de maximiser le pire des scores, plutôt que la moyenne des scores :
\begin{equation}
    \hat{\beta}, \hat{t} = \argmax_{\substack{t \in [0, 1] \\ \beta \in \R_+}}
    \min_{X \in \cal{X}} S(\beta, t; X)
\end{equation}

En pratique, pour constituer $\cal{X}$, on construit aléatoirement cinq sous-taxonomies de DBpédia, de différentes tailles et de différentes profondeurs.
...
On applique notre méthode d'extraction de taxonomie aux cinq sous-taxonomies contenues dans $\cal{X}$ pour vingt valeurs de $\beta$ réparties linéairement dans l'intervalle $[1, 200]$. Pour chacune des valeurs de $\beta$, on fait varier $t$ dans l'intervalle $[0, 1]$, ce qui nous permet d'obtenir empiriquement le seuil optimal $t^*(\beta)$. On obtient ainsi un tableau reliant chaque couple de paramètres $(\beta, t) \in [0, 200] \times [0, 1]$  et chaque jeu sous-taxonomie $X_k$ à la mesure $F_1$ moyenne associée $F_1^M(X_k, \beta, t)$.
